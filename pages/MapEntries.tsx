...
const localStyles = StyleSheet.create({
    ...
    header: {
        flexDirection: 'column', // Changed to column to stack title and search bar
        alignItems: 'center',
        justifyContent: 'space-between',
        paddingHorizontal: 15,
        paddingBottom: 10,
    },
    headerTopRow: { // New style for the top row with back button and title
        flexDirection: 'row',
        alignItems: 'center',
        justifyContent: 'space-between',
        width: '100%',
...
export default function MapEntriesScreen() {
    ...
    const fetchEntries = useCallback(async (page: number, query: string = '') => {
        if (!userToken) {
        ...
        try {
            ...
            setError(err.message || "Failed to load entries. Please check your network connection.");
        } finally {
            setIsLoading(false);
    ...
    const renderItem = ({ item }: { item: AreaEntry }) => (
        <TouchableOpacity
            ...
            onPress={() => Alert.alert("Entry Button Pressed", `You selected: ${item.Area_Name}`)}
        >
            ...
            <Text style={localStyles.cardDetails}>Coordinates: {item.coordinates.length}</Text>
        ...
        <View style={[localStyles.safeArea, { paddingTop: insets.top }]}>
            ...
            {isLoading && !isFetchingMore ? (
                <View style={[localStyles.loadingContainer, { flex: 1 }]}>
                    ...
                    <Text style={[Styles.text, { marginTop: 10, color: '#888' }]}>Loading entries...</Text>
                </View>
            ) : error ? (
                <View style={[localStyles.loadingContainer, { flex: 1 }]}>
                    <Text style={localStyles.errorText}>{error}</Text>
                    <TouchableOpacity onPress={() => fetchEntries(1)} style={{ marginTop: 10 }}>
                    ...
                    ListFooterComponent={() => (
                        hasMore ? (
                            <View style={localStyles.loadingContainer}>
                                <TouchableOpacity onPress={handleLoadMore} style={localStyles.loadMoreButton} disabled={isFetchingMore}>
                                    {isFetchingMore ? (
