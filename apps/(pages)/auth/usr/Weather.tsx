...
import { WeatherPreviewProps } from '../../../navigation/types';
...
const LineGraph = ({ data }: LineGraphProps) => {
  ...
  const temperatures = validData.map((d) =>
    parseFloat(d.values.temperature.toFixed(0)),
  );
  const labels = validData.map((d) =>
    new Date(d.startTime).toLocaleDateString('en-US', { weekday: 'short' }),
  );
...
export default function Weather({ route, navigation }: WeatherPreviewProps) {
  const [currentWeather, setCurrentWeather] = useState<WeatherValues | null>(
    null,
  );
          ...
          <View style={localStyles.contentContainer}>
            {currentWeather && (
              <View style={localStyles.weatherSummary}>
                ...
                <Text style={localStyles.summaryText}>Current Conditions</Text>
              </View>
            )}
                ...
                <View style={localStyles.detailsGrid}>
                  <View style={localStyles.detailItem}>
                    ...
                    <Text style={localStyles.detailValue}>
                      {currentWeather.humidity?.toFixed(0) ?? 'N/A'}%
                    </Text>
                  </View>
                  <View style={localStyles.detailItem}>
                    <Text style={localStyles.detailLabel}>Apparent Temp</Text>
                    <Text style={localStyles.detailValue}>
                      {currentWeather.temperatureApparent?.toFixed(1) ?? 'N/A'}
                      °C
                    </Text>
                  </View>
                  <View style={localStyles.detailItem}>
                    ...
                    <Text style={localStyles.detailValue}>
                      {currentWeather.precipitationProbability?.toFixed(0) ??
                        'N/A'}
                      %
                    </Text>
                  </View>
                  <View style={localStyles.detailItem}>
                    ...
                    <Text style={localStyles.detailValue}>
                      {currentWeather.windSpeed?.toFixed(1) ?? 'N/A'} m/s
                    </Text>
                  </View>
                  <View style={localStyles.detailItem}>
                    <Text style={localStyles.detailLabel}>Wind Direction</Text>
                    <Text style={localStyles.detailValue}>
                      {currentWeather.windDirection?.toFixed(0) ?? 'N/A'}°
                    </Text>
                  </View>
                  <View style={localStyles.detailItem}>
                    <Text style={localStyles.detailLabel}>Pressure</Text>
                    <Text style={localStyles.detailValue}>
                      {currentWeather.pressureSurfaceLevel?.toFixed(1) ?? 'N/A'}{' '}
                      hPa
                    </Text>
                  </View>
                </View>
                ...
                <View style={localStyles.dailyForecastList}>
                  {dailyForecast.length > 0 &&
                    dailyForecast.map((day, index) => (
                      <View
                        key={index}
                        style={[localStyles.dayCard, Styles.itemBackground]}
                      >
                        <Text style={localStyles.dayCardText}>
                          {new Date(day.startTime).toLocaleDateString()}
                        </Text>
                        <Text style={localStyles.dayCardText}>
                          {day.values.temperature?.toFixed(1) ?? 'N/A'}°C
                        </Text>
                      </View>
                    ))}
                </View>
